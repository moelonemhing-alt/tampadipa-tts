...
# ၃။ အမြန်နှုန်း (SPEED) နှင့် အသံအတက်အကျ (PITCH) ဆလိုက်ဒါ ချိန်ညှိမှုစနစ်
col_adj1, col_adj2 = st.columns(2)
with col_adj1:
    user_speed = st.slider("အမြန်နှုန်း (SPEED)", min_value=-10, max_value=10, value=-4, step=1)
with col_adj2:
    user_pitch = st.slider("အသံ အတက်/အကျ (PITCH)", min_value=-10, max_value=10, value=0, step=1)
...
if st.button("အသံထုတ်ယူမည်"):
    if not text.strip():
        ...
        with st.spinner("AI အသံနှင့် စာတန်းထိုး ပြုလုပ်နေပါသည်..."):
            ...
            final_pitch = vp["pitch"] + ep["pitch"] + (user_pitch * 3) # ၁ စကေးလျှင် 3% တိုး/လျှော့သည်
            final_rate = vp["rate"] + ep["rate"] + (user_speed * 5)   # ၁ စကေးလျှင် 5% တိုး/လျှော့သည်
            
            # edge-tts အတွက် လိုအပ်သော format ပုံစံပြောင်းခြင်း
            pitch_str = f"{final_pitch:+.0f}%"
